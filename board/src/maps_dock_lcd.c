...
static void MAPS_LCD_FlexBus_Init(void)
{
  ...
  FB_CSMR0 = (0 | FB_CSMR_BAM(FLEX_ADRESS_MASK) // Write FlexBus address mask
              | FB_CSMR_V_MASK                  // Selection signal available
  );

  FB_CSAR0 = (0 | FB_CSAR_BA(FLEX_BASE_ADDRESS)); // Write FlexBus address

  FB_CSCR0 = (0 | FB_CSCR_BLS_MASK // Bate-Lane Shift (right aligned mode)
  ...
  // SRAM configuration
  FB_CSMR2 = (0 | FB_CSMR_BAM(SRAM_ADRESS_MASK) // Write SRAM address mask
              | FB_CSMR_V_MASK                  // Selection signal available
  );

  FB_CSAR2 = (0 | FB_CSAR_BA(SRAM_BASE_ADDRESS)); // Write SRAM address

  FB_CSCR2 = (0 | FB_CSCR_BLS_MASK // Bate-Lane Shift(Right Alignment Mode)
              | FB_CSCR_AA_MASK    // Automatic response ACK
              | FB_CSCR_PS(2)      // 16Bit data transfer
  ...
  FB_CSPMCR = (0 | FB_CSPMCR_GROUP2(2) // Select FB_BE_31_24
