...
static void CAN_Set_Baud(CAN_CANn CAN_CANx, CAN_BAUD_n CAN_BAUD_x)
{
  ...
  CAN_CTRL1_REG(CANN[CAN_CANx]) &= ~(0 | CAN_CTRL1_PROPSEG_MASK | CAN_CTRL1_RJW_MASK | CAN_CTRL1_PSEG1_MASK | CAN_CTRL1_PSEG2_MASK | CAN_CTRL1_PRESDIV_MASK);

  CAN_CTRL1_REG(CANN[CAN_CANx]) |= (0 | CAN_CTRL1_PROPSEG(PCAN_Baud->Prop_Seg) | CAN_CTRL1_RJW(PCAN_Baud->RJW) | CAN_CTRL1_PSEG1(PCAN_Baud->PSeg1) | CAN_CTRL1_PSEG2(PCAN_Baud->PSeg2) | CAN_CTRL1_PRESDIV(PCAN_Baud->Presdiv));
...
void CAN_Init(CAN_CANn CAN_CANx, CAN_BAUD_n CAN_BAUD_x, CAN_MODE CAN_MODE_x, CAN_CLKSRC_n CAN_CLKSRC_x)
{
  ...
  while (CAN_MCR_REG(CANN[CAN_CANx]) & CAN_MCR_LPMACK_MASK)
    ;                                                  // Waiting for reset
...
void CAN_RxBuff_Enable(CAN_CANn CAN_CANx, MB_NUM_n MB_NUM_x, CAN_USR_ID_n CAN_USR_ID_x)
{
  ...
  if (CAN_USR_ID_x.IDE)
  {
    ...
    CAN_CS_REG(CANN[CAN_CANx], MB_NUM_x) = (0 | CAN_CS_IDE_MASK | (CAN_USR_ID_x.RTR << CAN_CS_IDE_SHIFT) | CAN_CS_CODE(CAN_CS_CODE_RX_EMPTY));
  }
  else
  {
    CAN_ID_REG(CANN[CAN_CANx], MB_NUM_x) = (0 | CAN_ID_STD(CAN_USR_ID_x.ID));

    CAN_CS_REG(CANN[CAN_CANx], MB_NUM_x) = (0 | (CAN_USR_ID_x.RTR << CAN_CS_IDE_SHIFT) | CAN_CS_CODE(CAN_CS_CODE_RX_EMPTY));
  }
}
