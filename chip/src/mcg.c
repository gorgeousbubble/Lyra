...
uint8 PLL_Init(PLLn PLLx)
{
  ...
  while (MCG_S & MCG_S_IREFST_MASK)
    ; // waiting to use external reference clock
  while (((MCG_S & MCG_S_CLKST_MASK) >> MCG_S_CLKST_SHIFT) != 0x2)
    ; // waiting for selection of external reference clock
  ...
  SIM_CLKDIV1 = SIM_CLKDIV1_OUTDIV1(MCG_DIV.SIM_CORE_DIV) | SIM_CLKDIV1_OUTDIV2(MCG_DIV.SIM_BUS_DIV) | SIM_CLKDIV1_OUTDIV3(MCG_DIV.SIM_FLEX_DIV) | SIM_CLKDIV1_OUTDIV4(MCG_DIV.SIM_FLASH_DIV);

  for (i = 0; i < MCG_DIV.SIM_FLASH_DIV; i++)
    ; // Wait
  ...
  while (!(MCG_S & MCG_S_PLLST_MASK))
    ; // waiting for PLL clock source selection
  ...
  while (((MCG_S & MCG_S_CLKST_MASK) >> MCG_S_CLKST_SHIFT) != 0x3)
    ; // waiting for PLL clock selection
