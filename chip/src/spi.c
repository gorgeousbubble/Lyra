...
uint32 SPI_Init(SPI_SPIn SPI_SPIx, SPI_PCSn SPI_PCSx, SPI_CFG SPI_cfg, uint32 SPI_Baud)
{
  ...
  {
    SPI_MCR_REG(SPIN[SPI_SPIx]) = (0 | SPI_MCR_MSTR_MASK     // Enable host mode
  ...
  SPI_CSSCK_EXIT:
    SPI_CTAR_REG(SPIN[SPI_SPIx], 0) = (0 | SPI_CTAR_PBR(SPI_Fit_Pbr)     // Baud rate pre division factor
  ...
  {
    SPI_CTAR_SLAVE_REG(SPIN[SPI_SPIx], 0) = (0 | SPI_CTAR_SLAVE_FMSZ(7) | SPI_CTAR_SLAVE_CPOL_MASK | SPI_CTAR_SLAVE_CPHA_MASK);
  }
