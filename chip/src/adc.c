...
static void ADC_Start(ADCn_CHn ADCn_CHx, ADC_nBit ADC_xBit)
{
  ...
  ADC_CFG1_REG(ADCN[ADCn(ADCn_CHx)]) = (0 | ADC_CFG1_ADIV(2)      // ADC clock divided by 4, 2 ^ n, n=0~3
  ...
  ADC_CFG2_REG(ADCN[ADCn(ADCn_CHx)]) = (0 | ADC_CFG2_ADHSC_MASK // High speed sampling conversion sequence
                                        | ADC_CFG2_ADLSTS(0)    // Default maximum sampling time, 24 ADCKs
  );

  // ADC Status Control Register (Channel A)
  ADC_SC1_REG(ADCN[ADCn(ADCn_CHx)], ADC_Channel_A) = (0 | ADC_SC1_AIEN_MASK             // Conversion completion interrupt enable
...
void ADC_Stop(ADC_ADCn ADC_ADCx)
{
  ...
  ADC_SC1_REG(ADCN[ADC_ADCx], ADC_Channel_A) = (0 | ADC_SC1_AIEN_MASK       // ADC conversion completed interrupt enable
